...
function Checkout({ cartItems, user, onBack, onCompleteCheckout }) {
  ...
  const validateValues = () => {
    ...
    if (!values.email.trim()) nextErrors.email = 'Ingresa un correo electronico.';
    if (values.email.trim() && !EMAIL_REGEX.test(values.email.trim())) {
      nextErrors.email = 'Ingresa un correo electronico valido.';
    }
    if (!values.phone.trim()) nextErrors.phone = 'Ingresa un numero de contacto.';
    if (!values.address.trim()) nextErrors.address = 'Ingresa la direccion de entrega.';
    if (!values.city.trim()) nextErrors.city = 'Ingresa la ciudad.';
    if (!values.postalCode.trim()) nextErrors.postalCode = 'Ingresa el codigo postal.';
    if (!values.shippingMethod) nextErrors.shippingMethod = 'Selecciona un metodo de envio.';
    if (!values.paymentMethod) nextErrors.paymentMethod = 'Selecciona un metodo de pago.';
      ...
      <section className={styles.container}>
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyText}>
            No hay productos en el carrito. Regresa para agregar articulos antes de continuar.
          </p>
